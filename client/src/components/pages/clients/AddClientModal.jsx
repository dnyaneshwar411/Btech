...
import { clientDetailsFields } from "@/utils/data/form-fields";
...
export default function AddClientModal() {
  ...
  const [formData, setFormData] = useState({ status: "active", type: "individual" })
  ...
  async function addClient() {
    try {
      setLoading(true);
      for (const field of ["name", "email", "phone", "address", "company_name", "status", "type",]) {
        if (!formData[field]) throw new Error(`Please enter the field - ${field}. It is compulsory!`);
      }
      const response = await sendData(`clients`, formData, "POST");
  ...
  return <Dialog>
    ...
    <DialogContent className="!max-w-[450px] text-center border-0 p-0 overflow-auto gap-0">
      <DialogHeader className="px-4 py-3 border-b-1">
        <DialogTitle className="text-[24px] p-0">Add Client</DialogTitle>
      </DialogHeader>
      <div className="px-4 py-4">
        {clientDetailsFields.map(field => selectComponent(field, formData, setFormData))}
        <Button
          onClick={addClient}
          disabled={loading}
          className="mt-10"
